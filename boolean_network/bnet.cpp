...
const std::vector<BnetNodeID>& BnetNode::getTruthTable() const {
    return truth_table_;
}

bool BnetNode::isOnSet() const {
    return is_onset_;
}

bool BnetNode::isInput() const {
    return is_input_;
}
...
BnetNetwork::BnetNetwork(const std::string &file, bool add_source_sink) {
    ...
    auto node_ls = this->getNodesList();
    for (auto &i : node_ls) {
        if (std::find(inputs_.begin(), inputs_.end(), i->getName()) != inputs_.end())
            continue;
        if (std::find(outputs_.begin(), outputs_.end(), i->getName()) != outputs_.end())
            continue;
        internals_.push_back(i->getName());
    }
}
...
const std::vector<BnetNodeID>& BnetNetwork::getInternalNames() const {
    return internals_;
}
